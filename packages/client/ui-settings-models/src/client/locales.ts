...
export const en = {
  ...
  disableTitle: 'Remove {provider}?',
  disableDescription: 'Removing {provider} takes its models out of the model picker. Its configuration and any stored API key are kept, and you can turn it back on from this page at any time.',
  disableConfirm: 'Remove {provider}',
  enable: 'Turn on',
  enableProvider: 'Turn on {provider}',
  providerOff: 'Off',
  ...
  addProviderOnboardingTitle: 'Add a provider to get started',
  addProviderOnboardingDescription: 'Name the provider, give its OpenAI-compatible endpoint, and paste its API key. Its models are read from that endpoint when you save.',
  addProviderOnboardingName: 'Name',
  addProviderOnboardingNamePlaceholder: 'Acme Gateway',
  addProviderOnboardingNoModels: 'The endpoint listed no model, so no provider was created. Check the address, or configure it on the Models page.',
...
export const zh: { [Key in keyof typeof en]: string } = {
  ...
  disableTitle: '移除 {provider}？',
  disableDescription: '移除 {provider} 会把它的模型从模型选择器中撤下；配置与已存储的 API 密钥都会保留，你可以随时在此页面重新开启。',
  disableConfirm: '移除 {provider}',
  enable: '开启',
  enableProvider: '开启 {provider}',
  providerOff: '已关闭',
  ...
  addProviderOnboardingTitle: '添加一个提供方即可开始',
  addProviderOnboardingDescription: '填写提供方名称、其 OpenAI 兼容端点与 API Key；保存时会从该端点读取模型列表。',
  addProviderOnboardingName: '名称',
  addProviderOnboardingNamePlaceholder: 'Acme 网关',
  addProviderOnboardingNoModels: '该端点未列出任何模型，因此未创建提供方。请检查地址，或在 Models 页面配置。',
