...
Models stay grouped by provider, with DeepSeek Account first and DeepSeek second; third-party providers retain their catalog order. The composer menu shows model and effort names only. The `/model` popup shows provider names and catalog descriptions; it localizes the two built-in DeepSeek descriptions and leaves external provider descriptions verbatim. Both surfaces submit, for the picked model, the effort this client last had in effect for that model — kept per provider/model across Sessions and page loads — and the model's own default when it has none, so switching to another model and back does not reset the level; the composer can then choose any advertised effort. A remembered level the model no longer advertises falls back to that default. An adapter without reasoning metadata leaves the Effort row absent; there is no arbitrary effort input.
...
Two entries over ONE per-session directory owned by `ModelDirectoryResolver` (`ctx.modelDirectories`): the `/model` popupSelect contribution (registered through `ctx.commandUi`) and the composer's named `conversation.input.model` seat both load the session's advisory directory through `session.models` and submit through `session.selectModel` via the same `ModelDirectory` instance, so a switch made in either entry is what the other shows next. Both resolve a row's effort through the same `ModelDirectory.selectionFor`, which reads one route-to-effort memory (`ModelEffortMemory`, localStorage entry `dsh.models.effort`) shared by every Session: the Session's own level wins for the route already in use, then the level remembered for the picked model while it still advertises it, then the model default. Directory loads and selections share a generation counter so an older response never overwrites a newer one. The directory publishes the latest submitted selection as `pending` until it settles or a connection reset invalidates it; a connection reset drops every resident projection and repulls the Host-restored selection before display. Directories are per-session, resolved lazily, and disposed with the session scope; addressed subagent sessions expose neither entry. Every resident directory refetches directly on forwarded `llm/adapters-updated`, `settings/document-updated`, and credential update events.
