...
export function apply(ctx: Context, config: Config): void {
  ...
  registerDeepSeekProvider(ctx, PROVIDER, {
    ...
    // A disabled provider holds no routes, which is what takes it out of the
    // picker and the provider directory while leaving its section and page row in place.
    enabled: () => plainOptions(config).enabled !== false,
  })
}
