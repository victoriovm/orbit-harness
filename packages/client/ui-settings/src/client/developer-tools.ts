...
import { booleanPreference, type BooleanPreference } from './preference.ts'
...
export class DeveloperToolsPreference {
  ...
  private readonly preference: BooleanPreference

  /**
   * @param scope - settings-owned namespace controller.
   */
  constructor(scope: ConfigForm<DeveloperToolsSettings>) {
    this.preference = booleanPreference(scope, 'enabled', { hostFallback: false, memoryStart: true })
    this.enabled = this.preference.value
  }
  ...
  async setEnabled(enabled: boolean): Promise<void> {
    await this.preference.set(enabled)
  }
}
